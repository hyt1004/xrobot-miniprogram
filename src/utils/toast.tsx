...
import { humanizeErrToMsg } from './exception'
...
export function useApiErrorToast() {
  ...
  const apiErrorToast = (error: any, defaultToastMsg?: string) => {
    let errMsg = defaultToastMsg || '请求失败'
    if (error instanceof CommonApiError) {
      errMsg = humanizeErrToMsg(error)
    }
    showToast({ tip: errMsg, icon: 'fail' })
  }
